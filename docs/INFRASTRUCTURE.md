...
| 2 | Obsidian Publish | The reading site at `social-research-methods.confused4now.org` | The book is offline |
...
- **Address:** <https://social-research-methods.confused4now.org>. Two earlier addresses, both dead as book addresses: `confused4now.org`, which is now the platform portal, and the staging domain `bptext2026.xyz` before that. Dates are in *The domain cutover* below
...
- **Two standing caveats**, both recorded in `suggest-edit-function/README.md`:
  ...
  - The accepted origin is no longer hardcoded: since migration step 2 the function
    matches the `Origin` header against `site.domain` in the registry it bundles at
    build time. A domain move is a registry change, and the function picks it up on
    the deploy that the merge triggers — but until that deploy lands, suggestions are
    refused on both the old and the new address.
